 ...
 *
 * One magic method to look at for how Exceptions are mapped to
...
public class LastStopHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleNoHandlerFoundException(NoHandlerFoundException ex, HttpHeaders headers,
        ...
        if(requestURL.contains("headers")) {
            return ResponseEntity.status(404).body(RestResultWrapper.ofError("Need to add headers"));
        } else {
            return ResponseEntity.status(404).body(RestResultWrapper.ofError("Resource not found:" + requestURL));
        }
    }
